...
include "../model/danhmuctacgia.php";
        ...
        <div class="row mt-5 main-web">
            ...
            if (isset($_GET['act']) && $_GET['act'] != "") {
                ...
                switch ($act) {
                    ...
                    case "dmtg":
                        $dmtg = danhmuc_tacgia();
                        include "tacgia/list-dm-tacgia.php";
                        break;

                    case "editdmtg":
                        if (isset($_GET['idtt']) & $_GET['idtt'] > 0) {
                            $danhmuctacgia = getone_dm_tacgia($_GET['idtt']);
                        }
                        if (isset($_POST['btnsubmit'])) {
                            $idtt = $_POST['idtt'];
                            $tentt = $_POST['tentt'];
                            update_dm_tacgia($idtt, $tentt);
                            header("location: ?act=dmtg");
                        }
                        include "tacgia/edit-dm-tacgia.php";
                        break;

                    case "adddmtg":
                        if (isset($_POST['btnsubmit'])) {
                            add_dm_tacgia($_POST['tentt']);
                            header("location: ?act=dmtg");
                        }
                        include "tacgia/add-dm-tacgia.php";
                        break;

                    case "deletetg":
                        if (isset($_GET['idtt']) && $_GET['idtt'] > 0) {
                            delete_dm_tacgia($_GET['idtt']);
                            header("location: ?act=dmtg");
                        }
                        break;
                    ...
                    case "dss":
                        $dstg = danhsach_sach();
                        include "sach/list-sach.php";
                        break;

                    case "addtg":
                        if (isset($_POST['btnsubmit'])) {
                            $tieudett = $_POST['tieudett'];
                            if ($_FILES['hinh_anh']['name'] != "") {
                                $photo = time() . "_" . $_FILES['hinh_anh']['name'];
                                move_uploaded_file($_FILES['hinh_anh']['tmp_name'], "../uploads/$photo");
                            }
                            $gia = $_POST['gia'];
                            $motatg = $_POST['motatg'];
                            $photo = null;
                            $danhmuctg = $_POST['danhmuctg'];
                            add_sach($tieudett, $photo, $gia, $motatg, $danhmuctg);
                            header("location: ?act=dss");
                        }
                        $dmtg = danhmuc_tacgia();
                        include "sach/add-sach.php";
                        break;

                    case "deletetg1":
                        // echo 123;
                        if (isset($_GET['idtg']) && $_GET['idtg'] > 0) {
                            delete_tacgia($_GET['idtg']);
                            header("location: ?act=dss");
                        }
                        break;
